...
cat("\nStep 3a completed: cleaned text and standardized years\n ") 

## 3) CONFIDENCE ASSIGNMENT
cat("\nStep 3b: Confidence assigment\
...
cat("\nStep 3b completed: confidence assigned\n ")

## 4) STANDARDIZE YEARS
cat("\nStep 3c: standardize years") 
...
dataset$confidenceFirstRecordEvent[matched_inds] <-
...
fwrite(non_matching_rows, "data/tmp/fr_non_matching_formats1.csv")

cat("\nStep 3c completed: years have been standardized and Non-matching rows pre-final cleaning \"fr_non_matching_formats1.csv\" available in tmp folder\n ")

## 5) FINAL CLEANING
cat("\nStep 3d: final cleaning")

clean_first_record <- function(dt) {
  dt[, firstRecordEvent := gsub("[^0-9\\-]", "", firstRecordEvent)]  # Keep only digits and -
  dt[, firstRecordEvent := gsub("(?<!\\d)(\\d{1,2})(?!\\d)",
      "", firstRecordEvent, perl = TRUE
    )
...
# Filter rows that DO NOT match this pattern
non_matching_rows <- dataset[!grepl(pattern, firstRecordEvent)]
fwrite(non_matching_rows, "tmp/fr_non_matching_formats2.csv")
