...
FR_years_0[, names(FR_years_0) := lapply(.SD, function(x) {
    ...
    x <- gsub("\\?", "", x, perl = TRUE)
    return(x)
    } else {return(x)}})] # nolint: indentation_linter.
...
## Delete empty rows
FR_years <- FR_years[verbatimEventDate != " "]
...
FR_years$FirstRecord <- str_replace_all(
  FR_years$FirstRecord,
  "(?<!\\d)(\\d{2})(?!\\d|\\s*(st|nd|rd|th)\\s+century)",
  function(x) {
...
mark_confidence <- function(dt) {
  ...
  dt[grepl("^\\d{4}$", FirstRecord), eventRemarks := "high confidence"]
  # Medium-high confidence: decades 
  dt[grepl("^\\d{4}s$|^\\d{2}s$", FirstRecord), eventRemarks := "medium-high confidence"]
  # Identify ranges
  range_rows <- grepl("^\\d{4}\\s*-\\s*\\d{4}$", dt$FirstRecord)
  if (any(range_rows)) {
    split_result <- tstrsplit(gsub("\\s+", "", dt$FirstRecord[range_rows]), "-", fixed = TRUE)
    if (length(split_result) == 2) {
      dt[range_rows, `:=`(
        range_start = as.numeric(split_result[[1]]),
        range_end   = as.numeric(split_result[[2]])
      )]     # Only assign if both parts are non-NULL
      dt[range_rows, range_width := abs(range_end - range_start)] # Calculate range width
      # Assign confidence based on range width
      ...
      dt[, c("range_start", "range_end", "range_width") := NULL] #remove temporary columns
    }
  }
...
convert_century_random <- function(text) {
  pattern <- regex("\\b(?:circa|ca|c\\.?|early|mid|late)?\\s*(\\d{1,2})(?:st|nd|rd|th)?\\s*(century|c)\\b", ignore_case = TRUE)
  matches <- str_match(text, pattern)
  if (!is.na(matches[1,1])) {
...
convert_after <- function(text) {
  pattern <- regex("(?i)(?:after\\s+|>\\s+)(\\d{4})(?:\\s+or\\s+later)?\\b")
  matches <- str_match(text, pattern)
  
  if (!is.na(matches[, 2])) {
    year <- as.numeric(matches[, 2])
    new_year <- year + sample(1:9, 1)
