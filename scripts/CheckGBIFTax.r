...
#  dat$GBIFusageKey <- NA
 ...
 # for (j in 1:n_taxa){# loop over all species names; takes some hours...
  for (j in 52300:n_taxa) {# loop over all species names; takes some hours...
    ...
    if (any(db$status=="ACCEPTED" & db$matchType=="EXACT" & colnames(db)=="canonicalName")) { 
      ...
      ## check alternative names #################################################################################
        ...
        if (nrow(alt_syn) == 0) {
          dat$GBIFnote[ind_tax] <- "No synonym with exact match found"
          next
        }
...
#      if (db_2$status=="ACCEPTED" & db_2$matchType=="EXACT"){
        if (nrow(db_2) > 0 && all(c("status", "matchType") %in% colnames(db_2))) {
          accepted_exact <- db_2[db_2$status == "ACCEPTED" & db_2$matchType == "EXACT", ]
        
          if (nrow(accepted_exact) > 0) {
            ...
            dat$GBIFnote[ind_tax] <- "Synonym returned no exact accepted match from GBIF"
          }
      } else {
        dat$GBIFnote[ind_tax] <- "Second GBIF query returned no results"
      }
      
      next # jump to next taxon

    } else {
      mismatches <- data.frame(Taxon = character(), status = character(), matchType = character(), stringsAsFactors = FALSE)
      mismatches <- rbind(mismatches, data.frame(
        Taxon = taxlist[j],
        status = if (!is.null(db$status)) as.character(db$status[1]) else NA_character_,
